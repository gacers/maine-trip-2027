...
//
// Tried and deliberately abandoned: chaining paid third-party scraping
// APIs (Apify, then Bright Data) as fallbacks when this direct fetch
// gets blocked. Confirmed live, with real sustained effort across both
// services, that Airbnb's harder-defended (professionally-managed)
// listings can defeat all of them the same way they defeat this — so
// that path bought unreliable results at real ongoing cost. The actual
// fix for those listings is components/BookmarkletButton.jsx: since
// the block is specific to automated *fetching*, not to the listing
// itself, reading the page from the visitor's own already-successful
// browser tab sidesteps the whole problem instead of trying to out-fetch it.
  ...
  if (!result.title) {
    result.warnings.push(
      blockedReason
        ? `${blockedReason}${
            isAirbnb
              ? " Use the bookmarklet below while viewing the real listing instead — it reads the page directly, so it can't be blocked."
              : " Double-check the link still works in your own browser, then fill in the fields below manually."
          }`
