...
// rotation, no managed browser, no vendor needed. See
// getAirbnbSessionCookie() below: an admin pastes their own session
// cookie into Settings (from their browser's own DevTools), and it's
// sent along with every Airbnb fetch here. It will eventually expire
// and need refreshing there — same page, no code change.
import { getAirbnbSessionCookie } from "@/lib/settings";
...
export async function scrapeListing(rawUrl) {
  ...
  try {
    ...
    if (isAirbnb) {
      const cookie = await getAirbnbSessionCookie();
    ...
    if (!res.ok) {
      ...
      if (html.length < 20000 && /404 Page Not Found|page you.re looking for/i.test(html)) {
        blockedReason = isAirbnb
          ? "Airbnb didn't return this listing's page — the saved session cookie may have expired (an admin can refresh it in Settings), or the listing may genuinely be gone."
