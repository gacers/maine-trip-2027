...
export async function exportSection(supabase, trip, section) {
  try {
    ...
    // a meaningful Rank — matches supports_ranking on the site itself.
    const showRank = !!section.supports_ranking;
    const header = buildHeader(overviewFields, showRank);
    ...
    const rows = units.map((u) => buildRow(u, overviewFields, trip, section, siteUrl, inviteToken, showRank));
...
function buildHeader(overviewFields, showRank) {
...
function buildRow(unit, overviewFields, trip, section, siteUrl, inviteToken, showRank) {
