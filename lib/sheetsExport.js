...
export async function exportSection(supabase, trip, section) {
  try {
    ...
    // anything to; only the Average Score (+ how many people rated) is a
    // real, static fact worth exporting.
    const showRank = !!section.supports_ranking;
    const showRatings = !!section.supports_ratings;
    const header = buildHeader(overviewFields, showRank, showRatings);
    ...
    const rows = units.map((u) =>
      buildRow(u, overviewFields, trip, section, siteUrl, inviteToken, showRank, showRatings)
    );
...
function buildHeader(overviewFields, showRank, showRatings) {
  ...
  if (showRatings) header.push("Average Score", "# Ratings");
...
function buildRow(unit, overviewFields, trip, section, siteUrl, inviteToken, showRank, showRatings) {
  ...
  if (showRatings) {
    ...
    row.push(unit.listings.map((l) => l.ratingCount ?? 0).join("\n"));
  }
