...
export async function exportSection(supabase, trip, section) {
  try {
    ...
    const header = buildHeader(overviewFields);
    ...
    const entries = rawEntries.map(toClientEntry);
    const sorted = [...entries].sort((a, b) => (a.rank ?? 999999) - (b.rank ?? 999999));
    const units = groupUnits(sorted).map((u) => ({
      ...u,
      rank: Math.min(...u.listings.map((l) => l.rank ?? 999999)),
    }));
    const rows = units.map((u) => buildRow(u, overviewFields, trip, section, siteUrl));
...
function buildHeader(overviewFields) {
  const header = ["Rank", "Property"];
...
function buildRow(unit, overviewFields, trip, section, siteUrl) {
  const anchor = unit.listings.length > 1 ? `group-${unit.listings[0].id}` : `listing-${unit.listings[0].id}`;
  const url = `${siteUrl}/${trip.slug}/${section.slug}#${anchor}`.replace(/"/g, '""');
  ...
  const row = [unit.rank >= 999999 ? "" : unit.rank, propertyCell];
  for (const f of overviewFields) {
