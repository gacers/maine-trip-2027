...
const LAST_COL = "O"; // must match HEADER.length (15 columns, A..O)
...
async function ensureSheet(sheets) {
  ...
  // Hide columns that are only useful internally / clutter the sheet:
  // id, posterImage, lat, lng, createdAt, siteLink. Grouped into
  // contiguous ranges where possible. Ignore failures (e.g. already
  // hidden) — cosmetic only.
  const hiddenRanges = [
    [0, 1], // id
    [5, 6], // posterImage
    [9, 11], // lat, lng
    [13, 15], // createdAt, siteLink
  ];
  try {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: {
        requests: hiddenRanges.map(([startIndex, endIndex]) => ({
