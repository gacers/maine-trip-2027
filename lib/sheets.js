...
// Groups items the same way the site does: two items sharing a
// non-empty groupLabel (a "2-house option", or any other paired option)
// become one display unit, so the Overview tab has one row per
// thing-you'd-actually-book, not one row per individual item.
...
function overviewRowForUnit(collection, unit) {
  return [
    ...
    unit.listings.map((l) => formatBedBath(l)).join("\n"),
...
export async function getOverviewSheetUrl(collection) {
  try {
