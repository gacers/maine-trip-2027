...
async function ensureOverviewSheet(sheets, collection) {
  ...
  // Overview name changed. Rename it in place (keeping its formatting,
  // gid, and any manual tweaks) rather than creating a fresh blank tab
  // and leaving the old one orphaned.
  if (!existing && collection.legacyOverviewSheetName) {
    const legacy = meta.data.sheets.find(
      (s) => s.properties.title === collection.legacyOverviewSheetName
    );
    if (legacy) {
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId,
