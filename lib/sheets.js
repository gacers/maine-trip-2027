...
// Light blue tint for an Overview row that bundles 2+ items into one
// option (see syncOverviewSheet), so it reads as one shared stay.
const GROUP_ROW_COLOR = { red: 0.91, green: 0.945, blue: 0.996 };
...
async function ensureOverviewSheet(sheets, collection) {
  ...
  // Always (re)apply currency formatting to the Avg/Night column — cheap
  // and idempotent, so this also backfills the format onto Overview tabs
  // that already existed before this column had one, not just brand-new
  // tabs. Formatting survives syncOverviewSheet's later values.clear()
  // calls (those only clear values, not formatting).
  const avgNightCol = OVERVIEW_HEADER.indexOf("Avg/Night");
  if (avgNightCol !== -1) {
    try {
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId,
          ...
          requests: [
            {
              repeatCell: {
                range: {
                  ...
                  startRowIndex: 1,
                  endRowIndex: 1000,
                  startColumnIndex: avgNightCol,
                  endColumnIndex: avgNightCol + 1,
                },
  ...
  // Sheets defaults new cells to bottom-vertical-alignment. That's fine
  // for single-line cells, but a 2-house option's multi-line cells (one
  // line per house) then get pushed to the BOTTOM of the row instead of
  // starting at the top — so a single-line cell in the same row (Rank)
  // visually lines up with the last house, making it look like the rank
  // only belongs to that one rather than to the whole grouped option.
  // Top-aligning everything makes the first line of every cell start
  // together, so a grouped row reads as one shared set of values.
  try {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: {
...
async function syncOverviewSheet(sheets, collection, items) {
  try {
    ...
    // Tint the whole row for a 2-house (or more) grouped option — beyond
    // the multi-line cells and shared Rank, a background color makes it
    // obvious at a glance that a row is one bundled stay rather than
    // several independent ones. Explicitly whiten solo rows too, in case
    // a row that used to be a group (and got tinted) isn't anymore now
    // that rows have been fully rewritten above.
    if (units.length > 0) {
      try {
        await sheets.spreadsheets.batchUpdate({
          spreadsheetId,
          requestBody: {
            requests: units.map((u, i) => ({
              repeatCell: {
                range: {
                  sheetId,
                  startRowIndex: i + 1,
                  endRowIndex: i + 2,
                  startColumnIndex: 0,
                  endColumnIndex: OVERVIEW_HEADER.length,
                },
                cell: {
                  userEnteredFormat: {
                    backgroundColor:
