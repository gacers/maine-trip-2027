  ...
  return [
    ...
    unit.listings
      .map((l) => {
        ...
        // Baked in as text rather than left as a real number + relying on
        // a cell-level currency format: a 2+ item group's cell here is a
        // "\n"-joined multi-line string (Sheets stores that as text, not
        // a number), so a numberFormat would silently never apply to it —
        // only ever visibly working for solo (single-value, real-number)
        // rows. Writing "$536" outright works the same either way.
