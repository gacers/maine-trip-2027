...
const OVERVIEW_HEADER = ["Rank", "Property", "Price", "Status", "Description", "Notes"];
const OVERVIEW_LAST_COL = "F";
  ...
  if (!existing) {
    ...
    if (currentHeader.join("|") !== OVERVIEW_HEADER.join("|")) {
      ...
      await sheets.spreadsheets.values.clear({
        spreadsheetId,
        range: `${OVERVIEW_SHEET_NAME}!A1:Z`,
      });
      await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: `${OVERVIEW_SHEET_NAME}!A1`,
...
function statusLabelFor(listing) {
  return listing.status === "archived"
    ? `Archived${listing.archiveReason ? ` (${listing.archiveReason.split(",").join(", ")})` : ""}`
    : "Active";
}

// Groups listings the same way the site does: two listings sharing a
// non-empty groupLabel (a "2-house option") become one display unit, so
// the Overview tab has one row per thing-you'd-actually-book, not one
// row per individual listing.
function groupForOverview(listings) {
  ...
  for (const l of sorted) {
    ...
    if (l.groupLabel) {
      ...
      if (partner) {
        seen.add(l.id);
        seen.add(partner.id);
        units.push({ rank: Math.min(l.rank ?? 999999, partner.rank ?? 999999), listings: [l, partner] });
        continue;
      }
    }
    seen.add(l.id);
    units.push({ rank: l.rank ?? 999999, listings: [l] });
  }
  return units;
}
...
// section anchor for a 2-house pair, or the single listing's own
// anchor otherwise).
function propertyCellValue(unit) {
  const anchorId =
    unit.listings.length > 1 ? `group-${unit.listings[0].id}` : `listing-${unit.listings[0].id}`;
  const url = siteAnchorUrl(anchorId).replace(/"/g, '""');
...
function overviewRowForUnit(unit) {
  return [
    unit.rank >= 999999 ? "" : unit.rank,
    propertyCellValue(unit),
...
async function syncOverviewSheet(sheets, listings) {
  try {
    ...
    const units = groupForOverview(listings);
    const rows = units.map(overviewRowForUnit);
...
// instead of writing the new listing there). Writing to an explicitly
...
export async function appendListing(listing) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `${SHEET_NAME}!A${nextRow}:${LAST_COL}${nextRow}`,
    valueInputOption: "USER_ENTERED",
    requestBody: { values: [listingToRow(listing)] },
  });
  const refreshed = await getAllListings();
